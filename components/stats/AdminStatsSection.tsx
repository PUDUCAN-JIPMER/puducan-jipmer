...
    // 1. PERFORMANCE: useMemo prevents re-calculating on every single render
    const filteredAndSortedStaffData = useMemo(() => {
        return stats.staffRoleData
            .filter(item => item.value > 0)
            .sort((a, b) => b.value - a.value)
    }, [stats.staffRoleData])
    
    const filteredAndSortedHospitalData = useMemo(() => {
        return stats.patientsPerHospital
            .filter(item => item.patients > 0)
            .sort((a, b) => b.patients - a.patients)
    }, [stats.patientsPerHospital])

    const activeAshaData = useMemo(() => {
        return stats.ashaCoverageData.filter(d => (d.covered + d.uncovered) > 0)
    }, [stats.ashaCoverageData])

    // 2. SCALABILITY: Calculate dynamic height based on number of items (35px per bar)
    // Ensures 5 items look normal, but 50 items get enough room to breathe
    const getDynamicHeight = (itemCount: number, minHeight = 220) => {
        return Math.max(minHeight, itemCount * 35)
    }
...
                    {/* SCALABILITY: Max height with vertical scrolling for 50+ items */}
                    <div className="max-h-87.5 w-full overflow-y-auto overflow-x-hidden pr-2 custom-scrollbar">
                        <ResponsiveContainer width="100%" height={getDynamicHeight(filteredAndSortedStaffData.length)}>
                            <BarChart 
...
                    <div className="max-h-87.5 w-full overflow-y-auto overflow-x-hidden pr-2 custom-scrollbar">
                        <ResponsiveContainer width="100%" height={getDynamicHeight(filteredAndSortedHospitalData.length)}>
                            <BarChart 
...
            {activeAshaData.length > 0 && (
                <ChartCard title="ASHA Coverage by Hospital">
                    <div className="max-h-100 w-full overflow-y-auto overflow-x-hidden pr-2 custom-scrollbar">
                        <ResponsiveContainer width="100%" height={getDynamicHeight(activeAshaData.length, 250)}>
                            <BarChart 
                                data={activeAshaData} 
                                layout="vertical"
                                margin={{ top: 5, right: 20, bottom: 5, left: 4 }}
...
                                <Bar dataKey="covered"   name="ASHA Assigned" stackId="a" fill="#4ade80" barSize={24} />
                                <Bar dataKey="uncovered" name="No ASHA"       stackId="a" fill="#f87171" radius={[0, 4, 4, 0]} barSize={24} />
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                </ChartCard>
